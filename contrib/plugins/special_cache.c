...
static Cache **pcaches;

static bool use_l2;
static Cache *scache;

static GMutex *pcache_locks;
static GMutex scache_locks;

static uint64_t instruction_count;

static uint64_t pcache_misses;
static uint64_t scache_misses;
...
static void vcpu_mem_access(unsigned int vcpu_index, qemu_plugin_meminfo_t info,
  ...
  g_mutex_lock(&pcache_locks[cache_idx]);
  hit_in_l1 = access_cache(pcaches[cache_idx], effective_addr);
  if (!hit_in_l1) {
    pcaches[cache_idx]->misses++;
  }
  pcaches[cache_idx]->accesses++;
  g_mutex_unlock(&pcache_locks[cache_idx]);
  ...
  g_mutex_lock(&scache_locks);
  if (!access_cache(scache, effective_addr)) {
    scache->misses++;
  }
  scache->accesses++;
  g_mutex_unlock(&scache_locks);
}
...
static void vcpu_insn_exec(unsigned int vcpu_index, void *userdata) {
  ...
  cache_idx = vcpu_index % cores;
  g_mutex_lock(&pcache_locks[cache_idx]);
  hit_in_l1 = access_cache(pcaches[cache_idx], insn_addr);
  if (!hit_in_l1) {
    pcaches[cache_idx]->misses++;
  }
  pcaches[cache_idx]->accesses++;
  ...
  g_mutex_lock(&scache_locks);
  if (!access_cache(scache, insn_addr)) {
    scache->misses++;
  }
  scache->accesses++;
  g_mutex_unlock(&scache_locks);
...
static void sum_stats(void) {
  ...
  g_assert(cores > 1);
  for (i = 0; i < cores; i++) {
    pcache_misses += pcaches[i]->misses;

    if (use_l2) {
      scache_misses += scache->misses;
    }
  }
}
...
static void log_stats(void) {
  ...
  for (i = 0; i < cores; i++) {
    ...
    icache = pcaches[i];
    l2_cache = scache;
    append_stats_line(rep, dcache->accesses, dcache->misses, icache->accesses,
                      icache->misses, l2_cache ? l2_cache->accesses : 0,
  ...
  if (cores > 1) {
    ...
    append_stats_line(rep, 0, l1_dmisses, l1_imem_accesses, l1_imisses,
